...
def show_help_dialog():
    ...
    # Список разделов синхронизирован с нумерацией в pages/
    topics = [
        "📥 Загрузка и Фильтры",
        "📊 1. Обзор выборки",
        "🧩 2. Анализ методик",
    ...
    with content_col:
        ...
        if selected_topic == topics[0]:
            ...
            st.markdown("---")
            st.markdown("#### 🧰 Полный список фильтров")
            st.markdown("""
            **Базовые (демография):** Пол · Возраст · Работа · Образование · КМНС (плюс конкретный народ) · Семейное положение · Дети · ВУЗ · Специальность · Уровень обучения · Основа обучения.
            """)

            st.markdown("""
            <div style="padding:12px; border-left: 5px solid #f39c12; background-color: #fef5e7; border-radius: 5px; margin-bottom: 10px;">
                <b>🛡️ Анти-фрод (Fast Clicker)</b><br>
                Отмечает респондентов, которые прошли опрос подозрительно быстро (вероятно, не вчитываясь).
                Оставьте пустым — увидите всех. Выберите <code>No</code> — останутся только «надёжные» ответы.
                Удобно для очистки данных перед публикацией.
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:12px; border-left: 5px solid #8e44ad; background-color: #f4ecf7; border-radius: 5px; margin-bottom: 10px;">
                <b>🧬 Фильтры по производным типам и профилям</b><br>
                Это категориальные результаты, рассчитанные из баллов:
                <ul style="margin: 8px 0;">
                <li><b>Мотивационный профиль</b> (Прогрессивный / Регрессивный / Импульсивный / Экспрессивный / Уплощённый) — отдельно для сфер «Жизнь» и «Работа»</li>
                <li><b>Эмоциональный профиль</b> (Стенический / Астенический / Смешанные)</li>
                <li><b>Стили ИПЛ</b>: поиска (ОИ/ФН), оценки (ПД/НГ), действия (ИП/ВП) + полный стиль</li>
                <li><b>Структура Г/А/П</b> (например, Г&lt;А&lt;П)</li>
                <li><b>Уровни Братуся</b> по 8 категориям (Доминирует / Нейтральный / Игнорируется)</li>
                </ul>
                Можно выделить, например, только «Прогрессивных + Стенических» — и сразу сравнить их с остальными во всех вкладках.
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; background:#eaf2f8; border-radius:5px; margin-top:10px;">
            <b>💡 Совет:</b> цепочка фильтров работает по принципу «И» — каждый следующий фильтр сужает выборку.
            После применения вверху страницы будет видно <i>«Отфильтровано: N из всего M»</i>.
            </div>
            """, unsafe_allow_html=True)
        ...
        # =========================================================
            ...
            st.markdown("#### 🛠️ Три подвкладки:")
            st.markdown("""
            * **👥 Социально-демографическая структура** — конструктор графиков по любому категориальному признаку (Pie / Bar / Treemap) + возрастная пирамида.
            * **📈 Описательная статистика и форма распределения** — Violin и Boxplot по любым числовым шкалам + подробная таблица (M, SD, медиана, асимметрия, эксцесс, проверка нормальности).
            * **🔀 Кросс-табуляция** — пересечение двух категориальных признаков (например, *Пол × Семейное положение*) в трёх режимах: проценты, абсолютные значения, тепловая карта.
            """)

            st.markdown("#### 📦 Как читать боксплот:")
            try:
                np.random.seed(7)
                demo_data = pd.DataFrame({
                    'Группа': ['Мужчины'] * 30 + ['Женщины'] * 30,
                    'Балл': np.concatenate([np.random.normal(55, 10, 30), np.random.normal(62, 8, 30)])
                })
                fig_demo = px.box(demo_data, x='Группа', y='Балл', color='Группа',
                                  color_discrete_map={'Мужчины': '#3498db', 'Женщины': '#e74c3c'},
                                  points='outliers', height=240)
                fig_demo.update_layout(showlegend=False, margin=dict(l=10, r=10, t=10, b=10))
                st.plotly_chart(fig_demo, use_container_width=True)
            except Exception:
                pass
            st.caption("Линия в середине ящика — медиана. Сам ящик — где находится средние 50% людей (от 25-го до 75-го перцентиля). Усы — основной разброс. Точки за усами — выбросы (атипичные ответы).")
        ...
        # =========================================================
            ...
            st.markdown("#### 🔍 Как читать профиль Мильмана (живой пример):")
            try:
                scales = ['Поддержание', 'Комфорт', 'Статус', 'Общение', 'Активность', 'Творчество', 'Общ. польза']
                ideal = [12, 15, 14, 18, 17, 19, 16]
                real = [10, 9, 12, 14, 10, 8, 11]

                fig_radar = go.Figure()
                fig_radar.add_trace(go.Scatterpolar(
                    r=ideal + [ideal[0]], theta=scales + [scales[0]],
                    line=dict(color='#27ae60', width=2, dash='dot'),
                    name='Идеал (желаемое)', mode='lines+markers',
                ))
                fig_radar.add_trace(go.Scatterpolar(
                    r=real + [real[0]], theta=scales + [scales[0]],
                    fill='toself', fillcolor='rgba(231, 76, 60, 0.15)',
                    line=dict(color='#e74c3c', width=2),
                    name='Реальное', mode='lines+markers',
                ))
                fig_radar.update_layout(
                    polar=dict(radialaxis=dict(visible=True, range=[0, 22], showticklabels=False)),
            ...
            * 🔴 **Красная сплошная линия:** Действительное (Реальность).
            ...
            <div style="padding:15px; border-left: 5px solid #e74c3c; background-color: #fadbd8; border-radius: 5px; margin-bottom: 15px;">
                ...
                Чем дальше красная точка от зеленой на одной шкале, тем выше уровень неудовлетворенности (фрустрации) в этой сфере. На примере выше максимальная фрустрация — по шкале «Творчество» (разрыв 11 баллов). Система автоматически рассчитает и подсветит шкалу с максимальной болью.
            </div>
            """, unsafe_allow_html=True)
        ...
        # =========================================================
            ...
            st.markdown("### 🆚 Вкладка 3: Сравнение групп и проверка гипотез")
            st.markdown("Пять разных режимов сравнения — для одного показателя, для целого профиля методики, для всех шкал сразу, для двух категорий, и для всех категорий сразу.")

            st.info("⚙️ **Под капотом:** Система сама проверяет нормальность распределения и выбирает критерий (t-Welch или U-Манна-Уитни для 2 групп; ANOVA или Kruskal-Wallis для 3+ групп). Вам не нужно знать математику.")

            st.markdown("#### 📑 Пять подвкладок:")

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #3498db; background:#ebf5fb; margin-bottom:8px; border-radius:5px;">
                <b>🎯 Детальный анализ одной шкалы</b><br>
                Выбираете одну шкалу + одну группирующую переменную → получаете один тест с p-value, размером эффекта, боксплотом и средними. <i>Когда применять: проверяете одну конкретную гипотезу.</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #9b59b6; background:#f4ecf7; margin-bottom:8px; border-radius:5px;">
                <b>📊 Сравнение профилей методик</b><br>
                Визуальное сравнение всех шкал одной методики между группами. Три внутренние подвкладки для Мильмана: 🏠 Жизнь / 💼 Работа / 🎭 Эмоции. <i>Когда применять: хотите целостную картину «как одна группа отличается от другой по всему мотивационному профилю».</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #27ae60; background:#e8f8f5; margin-bottom:8px; border-radius:5px;">
                <b>💡 Авто-поиск различий (число × группа)</b><br>
                Главный «сканер». Запускает тест по всем числовым шкалам сразу. Возвращает таблицу, отсортированную по p-value, с возможностью скачать в Excel.
                <i>Когда применять: «у меня есть деление по полу — где они различаются?»</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #e67e22; background:#fdf2e9; margin-bottom:8px; border-radius:5px;">
                <b>🔀 Пересечение типов (категория × категория)</b><br>
                Хи-квадрат тест связи между двумя категориальными переменными (например, «Мотивационный профиль × Структура ИПЛ»). Четыре режима таблицы: наблюдаемые, проценты, ожидаемые, остатки.
                <i>Когда применять: «связан ли мотивационный профиль с типом ИПЛ?»</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #c0392b; background:#fadbd8; margin-bottom:12px; border-radius:5px;">
                <b>🔎 Авто-поиск пересечений типов</b><br>
                Массовый хи-квадрат тест по всем парам категориальных переменных. Возвращает таблицу всех значимых связей с эффектом Крамера V.
                <i>Когда применять: разведочный анализ — «между какими типами вообще есть связи?»</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("#### Как читать звёздочки значимости:")
            c1, c2 = st.columns(2)
            with c1:
                st.markdown("<h4 style='text-align: center; color: #27ae60;'>p < α</h4>", unsafe_allow_html=True)
                st.markdown("<p style='text-align: center;'><b>Различия есть.</b> Это не случайность.<br><span style='color:#888;'>* p&lt;0.05 · ** p&lt;0.01 · *** p&lt;0.001</span></p>", unsafe_allow_html=True)
            with c2:
        ...
        # =========================================================
            ...
            st.markdown("#### 🛡 Поправка на множественные сравнения (важно!)")
            st.markdown("""
            При большом числе попарных сравнений часть «значимых» связей появляется случайно.
            Например, при 20 переменных у вас будет **190 пар** — даже на абсолютно случайных данных
            около 10 из них окажутся «значимыми» на уровне p<0.05 чисто по теории вероятности.
            """)
            colf1, colf2, colf3 = st.columns(3)
            with colf1:
                st.markdown("<div style='padding:10px; background:#fadbd8; border-radius:8px; text-align:center;'><b>Без поправки</b><br><span style='font-size:24px; color:#c0392b;'>28</span><br><span style='color:#888;'>«значимых» связей</span></div>", unsafe_allow_html=True)
            with colf2:
                st.markdown("<div style='padding:10px; background:#fef9e7; border-radius:8px; text-align:center;'><b>FDR (Бенджамини-Хохберг)</b><br><span style='font-size:24px; color:#d68910;'>12</span><br><span style='color:#888;'>остались значимыми</span></div>", unsafe_allow_html=True)
            with colf3:
                st.markdown("<div style='padding:10px; background:#d5f5e3; border-radius:8px; text-align:center;'><b>Бонферрони</b><br><span style='font-size:24px; color:#229954;'>7</span><br><span style='color:#888;'>самые надёжные</span></div>", unsafe_allow_html=True)

            st.info("""
            **🎯 Рекомендация:**
            — **FDR (по умолчанию)** — оптимально для большинства задач, стандарт в современной психологии.
            — **Бонферрони** — самая строгая, для подтверждающих исследований где цена ошибки высока.
            — **Без поправки** — только для разведочного анализа, когда вы планируете дальше всё проверить отдельно.
            """)
            st.caption("При выборе поправки таблица и фильтр связей внизу страницы автоматически пересчитываются. В подсказке к каждой ячейке тепловой карты показываются оба p-value: сырое и скорректированное.")
        ...
        # =========================================================
            ...
            st.markdown("Разделение людей на схожие типажи с помощью машинного обучения. "
                        "Оба метода (иерархический и K-Means) показывают **коэффициент силуэта** — "
                        "индикатор качества разбиения от −1 до 1 (чем выше, тем чётче разделены группы).")

            st.markdown("#### 👀 Что такое «хороший» и «плохой» силуэт визуально:")
            try:
                np.random.seed(42)
                # Хорошие кластеры
                good_x = np.concatenate([np.random.normal(-2, 0.5, 30), np.random.normal(2, 0.5, 30)])
                good_y = np.concatenate([np.random.normal(0, 0.5, 30), np.random.normal(0, 0.5, 30)])
                good_c = ['A'] * 30 + ['B'] * 30
                # Плохие кластеры
                bad_x = np.concatenate([np.random.normal(0, 1.5, 30), np.random.normal(0.5, 1.5, 30)])
                bad_y = np.concatenate([np.random.normal(0, 1.5, 30), np.random.normal(0.5, 1.5, 30)])
                bad_c = ['A'] * 30 + ['B'] * 30

                colg, colb = st.columns(2)
                with colg:
                    fig_g = px.scatter(x=good_x, y=good_y, color=good_c, height=220,
                                       color_discrete_map={'A': '#27ae60', 'B': '#3498db'})
                    fig_g.update_layout(showlegend=False, margin=dict(l=10, r=10, t=30, b=10),
                                        title=dict(text='Силуэт ≈ 0.72 (отлично)', x=0.5, font=dict(size=13)))
                    fig_g.update_xaxes(showticklabels=False)
                    fig_g.update_yaxes(showticklabels=False)
                    st.plotly_chart(fig_g, use_container_width=True)
                with colb:
                    fig_b = px.scatter(x=bad_x, y=bad_y, color=bad_c, height=220,
                                       color_discrete_map={'A': '#27ae60', 'B': '#3498db'})
                    fig_b.update_layout(showlegend=False, margin=dict(l=10, r=10, t=30, b=10),
                                        title=dict(text='Силуэт ≈ 0.10 (плохо)', x=0.5, font=dict(size=13)))
                    fig_b.update_xaxes(showticklabels=False)
                    fig_b.update_yaxes(showticklabels=False)
                    st.plotly_chart(fig_b, use_container_width=True)
            except Exception:
                pass

            st.info("""
            **📏 Шкала интерпретации силуэта:**
  ...
  Самый информативный формат: сразу видно, какие группы людей имеют какой профиль и по каким шкалам они различаются.""")
        ...
        # =========================================================
            ...
            **Три слоя проверки** перед запуском FA:
            ...
            st.markdown("#### 📉 Scree plot и поиск «локтя»:")
            try:
                comps = [1, 2, 3, 4, 5, 6, 7, 8]
                eigen = [3.8, 2.1, 1.3, 0.85, 0.6, 0.4, 0.3, 0.2]
                fig_scree = go.Figure()
                fig_scree.add_trace(go.Scatter(x=comps, y=eigen, mode='lines+markers',
                                               line=dict(color='#2980b9', width=3),
                                               marker=dict(size=10)))
                fig_scree.add_hline(y=1, line_dash='dash', line_color='#e74c3c',
                                    annotation_text='Критерий Кайзера (λ=1)',
                                    annotation_position='top right')
                fig_scree.add_annotation(x=3, y=1.3, text='👈 «локоть»<br>(оптимум = 3 фактора)',
                                         showarrow=True, arrowhead=2, ax=60, ay=-30,
                                         font=dict(size=11, color='#27ae60'))
                fig_scree.update_layout(height=260, margin=dict(l=40, r=10, t=10, b=40),
                                        xaxis_title='Номер компоненты', yaxis_title='Собственное значение (λ)')
                st.plotly_chart(fig_scree, use_container_width=True)
            except Exception:
                pass
            st.caption("Берите столько факторов, сколько умещается выше красной линии (λ > 1) или до «локтя» — точки, где график резко выравнивается.")

            st.markdown("""
            **Внутренние вкладки:**
            * **PCA** — быстрая первичная компоновка; scree plot + критерий Кайзера подскажут число факторов.
        ...
        # =========================================================
            ...
            try:
                drivers_demo = pd.DataFrame({
                    'Фактор': ['Творческая активность', 'Общественная польза', 'Комфорт', 'Поддержание', 'Статус'],
                    'Важность': [0.32, 0.24, 0.18, 0.16, 0.10],
                    'Направление': ['📈 Катализатор', '📈 Катализатор', '📉 Блокатор', '📉 Блокатор', '📈 Катализатор']
                })
                fig_imp = px.bar(drivers_demo, x='Важность', y='Фактор', orientation='h',
                                 color='Направление',
                                 color_discrete_map={'📈 Катализатор': '#27ae60', '📉 Блокатор': '#e74c3c'},
                                 height=240)
        ...
        # =========================================================
        # 8. РЕГРЕССИЯ (НОВЫЙ РАЗДЕЛ)
        # =========================================================
        elif selected_topic == topics[8]:
            st.markdown("### 📈 Вкладка 8: Регрессионный анализ")
            st.markdown("Строит модели предсказания целевой переменной четырьмя разными способами — от простых до продвинутых.")

            st.info("⚖️ **Чем отличается от вкладки «Поиск драйверов»?** Драйверы — это Random Forest, ищет нелинейные паттерны и важность факторов. Регрессия — это классические статистические модели с β-коэффициентами, p-value и доверительными интервалами. Именно регрессия — стандарт в публикациях.")

            st.markdown("#### 📑 Четыре подвкладки:")

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #3498db; background:#ebf5fb; margin-bottom:8px; border-radius:5px;">
                <b>🎯 Множественная регрессия (стандартная)</b><br>
                Базовый OLS. Выбираете Y и набор X. Получаете R², R²<sub>adj</sub>, F-тест модели, β-коэффициенты с их p-value и доверительными интервалами.
                <i>Когда применять: когда есть список потенциальных предикторов и нужно понять, кто из них значим.</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #8e44ad; background:#f4ecf7; margin-bottom:8px; border-radius:5px;">
                <b>🧱 Иерархическая регрессия (по блокам)</b><br>
                Стандарт в психологических исследованиях. Разбиваете предикторы на блоки (например: Блок 1 — демография, Блок 2 — смыслы, Блок 3 — мотивация) и добавляете их по очереди.
                Показывает <b>ΔR²</b> — сколько дополнительной дисперсии Y объясняет каждый блок сверх предыдущих.
                <i>Когда применять: «Объясняет ли мотивация ИПЛ сверх того, что уже объясняется смыслами и демографией?»</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #16a085; background:#e8f8f5; margin-bottom:8px; border-radius:5px;">
                <b>🕸 Частные корреляции</b><br>
                Связь между X и Y <b>при контроле других переменных</b>. Например, связь Братуся с ИПЛ может оказаться видимостью — на самом деле и то и другое связано с мотивацией.
                <i>Когда применять: «эта связь настоящая, или это эффект третьей переменной?»</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:10px; border-left: 4px solid #d35400; background:#fdf2e9; margin-bottom:12px; border-radius:5px;">
                <b>🛡 Регуляризованная регрессия (Ridge / Lasso)</b><br>
                Для случаев, когда предикторов много и они коррелируют между собой (мультиколлинеарность). <b>Ridge</b> сжимает все коэффициенты пропорционально, <b>Lasso</b> обнуляет самые слабые — делает автоматический отбор переменных.
                <i>Когда применять: предикторов больше 10, и есть подозрение что они дублируют друг друга.</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("#### 📋 Как выглядит результат (мини-таблица):")
            try:
                reg_demo = pd.DataFrame({
                    'Предиктор': ['Творческая активность', 'Общественная польза', 'Статус', 'Возраст'],
                    'β': [0.42, 0.31, -0.18, 0.05],
                    'SE': [0.08, 0.09, 0.07, 0.06],
                    't': [5.25, 3.44, -2.57, 0.83],
                    'p': ['< .001 ***', '.001 **', '.011 *', '.405'],
                })
                st.dataframe(reg_demo, hide_index=True, use_container_width=True)
            except Exception:
                pass
            st.caption("Знак β показывает направление влияния, его величина — силу. Звёздочки рядом с p — стандартное обозначение значимости.")

        # =========================================================
        # 9. ПОИСК АНОМАЛИЙ
        # =========================================================
        elif selected_topic == topics[9]:
            st.markdown("### 👽 Вкладка 9: Поиск аномалий")
            st.markdown("Поиск респондентов с нетипичными профилями (выбросов).")

            st.markdown("#### 🔀 Выбор метода:")

            st.markdown("""
            <div style="padding:12px; border-left: 5px solid #8e44ad; background: #f4ecf7; margin-bottom: 10px; border-radius:5px;">
                <b>🌲 Isolation Forest (ML, разведочный)</b><br>
                Алгоритм машинного обучения. Высокий мотив 'Комфорт' — норма. Высокое 'Творчество' — норма. Но если у человека зашкаливают <b>оба</b> взаимоисключающих мотива, алгоритм пометит его как аномалию.
                Хорош для поиска интересных кейсов, но <b>не имеет строгого статистического основания</b>.
                <i>Параметр чувствительности: какой процент выборки считать «странным» (рекомендуется 5%).</i>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("""
            <div style="padding:12px; border-left: 5px solid #2980b9; background: #ebf5fb; margin-bottom: 10px; border-radius:5px;">
                <b>📐 Расстояние Махаланобиса (классический, академический)</b><br>
                Классический статистический метод, основанный на χ²-распределении. Для каждого респондента считается, насколько он удалён от центра «облака» данных с учётом ковариаций.
                <ul style="margin:8px 0;">
                <li><b>Строгий критерий:</b> p &lt; 0.001 (рекомендуется для академической работы)</li>
                <li><b>Мягкий критерий:</b> p &lt; 0.01 (для предварительного просмотра)</li>
                </ul>
                Это <b>стандартный</b> метод подготовки данных к регрессионному и факторному анализу (Tabachnick &amp; Fidell, 2019). Используется в психометрии и SEM.
            </div>
            """, unsafe_allow_html=True)

            st.markdown("#### 👀 Как выглядит выброс на графике:")
            try:
                np.random.seed(11)
                normal_x = np.random.normal(0, 1, 60)
                normal_y = np.random.normal(0, 1, 60)
                anom_x = [3.2, -2.8, 2.9]
                anom_y = [-3.0, 2.5, 2.8]
                fig_anom = go.Figure()
                fig_anom.add_trace(go.Scatter(x=normal_x, y=normal_y, mode='markers',
                                              marker=dict(color='#95a5a6', size=8),
                                              name='Норма'))
                fig_anom.add_trace(go.Scatter(x=anom_x, y=anom_y, mode='markers',
                                              marker=dict(color='#e74c3c', size=14,
                                                          line=dict(color='#c0392b', width=2)),
                                              name='Аномалия'))
                fig_anom.update_layout(height=250, margin=dict(l=10, r=10, t=10, b=10),
                                       legend=dict(orientation='h', yanchor='bottom', y=-0.2),
                                       xaxis_title='', yaxis_title='')
                fig_anom.update_xaxes(showticklabels=False)
                fig_anom.update_yaxes(showticklabels=False)
            ...
            st.markdown("""
            1. Задайте параметры и запустите поиск.
        ...
        # =========================================================
            ...
            st.markdown("### 🕸️ Вкладка 10: Сетевой анализ")
            st.markdown("Визуализация структуры мотивов в виде графа.")

            st.markdown("#### 🌐 Мини-пример графа:")
            try:
                # Маленький граф для иллюстрации
                node_x = [0, 1, 2, 1, -1, -1, 2]
                node_y = [0, 1, 0, -1, 1, -1, -1]
                node_labels = ['Творчество', 'Активность', 'Общение', 'Статус', 'Комфорт', 'Поддержание', 'ИПЛ']
                node_size = [40, 35, 30, 25, 25, 25, 45]
                # Рёбра: (i, j, цвет)
                edges = [
                    (0, 1, 'green'), (0, 6, 'green'), (1, 2, 'green'),
                    (1, 6, 'green'), (2, 3, 'green'), (4, 5, 'green'),
                    (0, 4, 'red'), (1, 5, 'red'),
                ]
                fig_net = go.Figure()
                for i, j, c in edges:
                    color = '#27ae60' if c == 'green' else '#e74c3c'
                ...
                fig_net.add_trace(go.Scatter(
                    ...
                    marker=dict(size=node_size, color='#3498db', line=dict(color='white', width=2)),
                    text=node_labels, textposition='middle center',
                    textfont=dict(color='white', size=9),
                    hoverinfo='text', showlegend=False
                ))
                fig_net.update_layout(height=300, margin=dict(l=10, r=10, t=10, b=10),
        ...
        # =========================================================
        # 11. МЕДИАЦИЯ (НОВЫЙ РАЗДЕЛ)
        # =========================================================
        elif selected_topic == topics[11]:
            st.markdown("### 🔀 Вкладка 11: Медиационный анализ")
            st.markdown("Проверка гипотезы «X влияет на Y через M» — один из самых сильных инструментов причинно-следственного анализа в психологии.")

            st.markdown("#### 🧭 Что такое медиация:")
            st.markdown("""
            В простой модели **X → Y** вы проверяете прямую связь. Но часто за ней скрывается промежуточное звено — **медиатор M**. Модель становится:
            """)
            ...
            <svg width="460" height="180" xmlns="http://www.w3.org/2000/svg">
              <!-- Узлы -->
              <circle cx="60"  cy="140" r="32" fill="#3498db" stroke="#2980b9" stroke-width="2"/>
              <text x="60"  y="146" font-size="22" font-weight="bold" fill="white" text-anchor="middle">X</text>
              <circle cx="230" cy="40"  r="32" fill="#9b59b6" stroke="#7d3c98" stroke-width="2"/>
              <text x="230" y="46"  font-size="22" font-weight="bold" fill="white" text-anchor="middle">M</text>
              <circle cx="400" cy="140" r="32" fill="#27ae60" stroke="#1e8449" stroke-width="2"/>
              <text x="400" y="146" font-size="22" font-weight="bold" fill="white" text-anchor="middle">Y</text>
              <!-- Стрелки -->
              <defs>
              ...
              <line x1="88"  y1="125" x2="200" y2="55"  stroke="#555" stroke-width="2" marker-end="url(#arrow)"/>
              <text x="130" y="80"  font-size="18" font-style="italic" font-weight="bold" fill="#7d3c98">a</text>
              <line x1="258" y1="55"  x2="372" y2="125" stroke="#555" stroke-width="2" marker-end="url(#arrow)"/>
              <text x="320" y="80"  font-size="18" font-style="italic" font-weight="bold" fill="#7d3c98">b</text>
              <line x1="98"  y1="140" x2="362" y2="140" stroke="#555" stroke-width="2" stroke-dasharray="6,3" marker-end="url(#arrow)"/>
              <text x="225" y="160" font-size="16" font-style="italic" fill="#888" text-anchor="middle">c' (прямой эффект)</text>
            </svg>
            </div>
            """, unsafe_allow_html=True)
            ...
            - Связь: **c = c' + a·b** — общий эффект раскладывается на прямой и косвенный
            """)

            st.markdown("#### 📊 Пять возможных исходов:")
            try:
                med_outcomes = pd.DataFrame({
                    'Паттерн': ['Полная медиация', 'Частичная медиация', 'Прямой эффект без медиации', 'Нет эффекта', 'Супрессия'],
                    'a·b': ['значим', 'значим', '≈ 0', '≈ 0', 'значим'],
                    "c'": ['≈ 0', 'значим', 'значим', '≈ 0', 'значим, противоп. знак'],
                    'Интерпретация': [
                        'X влияет на Y ТОЛЬКО через M',
                        'X влияет и напрямую, и через M',
                        'Медиатор не нужен',
                        'Связи не существует',
                        'Медиатор маскировал связь (редкий случай)',
                    ]
                })
                st.dataframe(med_outcomes, hide_index=True, use_container_width=True)
            except Exception:
                pass

            st.markdown("#### 🛠️ Когда применять")
            st.markdown("""
            ...
            Дашборд считает значимость косвенного эффекта (a·b), показывает все коэффициенты с p-value и автоматически интерпретирует паттерн.
            """)
            st.info("📌 Перед запуском медиации полезно сначала пройтись по вкладкам **Корреляции** и **Регрессия** — чтобы убедиться, что нужные связи в принципе есть.")
